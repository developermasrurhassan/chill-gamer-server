...
async function run() {
    try {
        ...
        // Test connection by counting documents (safe operation)
        try {
            const gamesCount = await gamesCollection.countDocuments();
            const reviewsCount = await reviewsCollection.countDocuments();
            console.log(`Current data: ${gamesCount} games, ${reviewsCount} reviews`);
        } catch (countError) {
            console.log("Could not count documents (permission issue), but continuing...");
        }
